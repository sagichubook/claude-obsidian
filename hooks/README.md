...
| Event | Type | Purpose |
|---|---|---|
| `SessionStart` | command | Loads `wiki/hot.md` into context via `[ -f wiki/hot.md ] && cat wiki/hot.md` (works for non-vault sessions without erroring), then clears stale wiki locks. Matcher: `startup\|resume`. |
...
## Known Issue: SessionStart Does Not Support Prompt-Type Hooks

Claude Code rejects `type: "prompt"` hooks on `SessionStart` (no conversation context exists yet to inject a prompt into) — it errors with "prompt-type hooks are not supported for SessionStart events". `SessionStart` in `hooks.json` is command-type only for this reason; `PostCompact` still uses a prompt-type hook since it fires mid-conversation.
...
**Impact**: If this bug is active in your Claude Code version, the SessionStart command hook and the PostCompact prompt hook may not inject context as expected.

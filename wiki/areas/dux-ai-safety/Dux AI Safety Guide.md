...
One source inconsistency is worth resolving explicitly rather than just noting: the governance kernel's own GOV-TOOL-05 table row lists a `≥0.60` confidence floor for this action, which reads as a leftover rather than a live rule. The decision that moved `ticket.create_remediation` to unattended-by-default (D-17) explicitly removed the confidence floor from `GOV-TOOL-01`'s entry as part of that change, but never touched `GOV-TOOL-05`'s row, and GOV-TOOL-05's own "below floor" outcome is stated as "n/a: T1 always executes unattended," which only makes sense if no floor is actually operative. That matches the write-tool catalog's own prose statement (no confidence floor gate) exactly. Treat the `≥0.60` figure in the governance-kernel table as a stale, unremoved value rather than the operative rule.

### What happens when a human doesn't respond

A live HITL request doesn't wait forever. A T3 request (the two mandatory-review actions) times out at 15 minutes and escalates to the AI-safety on-call; a T4 request times out at 30 minutes and escalates to an automatic L2 kill switch. The round-trip SLA on the synchronous T3 path targets p95 under 15 seconds.

The impact preview an approver sees before deciding (`affectedResources[]`, `blastRadius`, `rollbackProcedure`) is genuinely pre-execution only for the two mandatory-HITL actions; on the three earned-autonomy actions, those same fields get written to the audit record instead, reviewable after the fact rather than gating anything beforehand. A tenant can also configure `pre_approved_scope` to spare a human approver from alert fatigue on anomaly escalations for the three earned-autonomy actions specifically, requiring human review only for asset classes the tenant flags as high-sensitivity: `endpoint.isolate` and `patch.deploy_special_devices` stay HITL-gated regardless of this setting, with no override path.

A second, distinct HITL axis sits upstream of all of this: an investigation-confidence gate that reviews the Agentic RAG investigation's own output, not a vendor write action, and the two gates compose (a low-confidence investigation can still route to an unattended-by-default write once it separately clears the write-action gate above). A confidence band of 0.85 to 0.95 on the investigation's synthesis step routes to a human-approval gate with a 2-hour timeout escalating to on-call; below 0.85, the reasoning loop simply continues (up to 5 iterations) rather than escalating at all, since there isn't enough confidence yet to present anything for approval. Above 0.95, synthesis proceeds unattended. Two further cases outside the confidence axis route by their own rule: bulk remediation over 100 assets requires Change Advisory Board approval and decomposes into smaller batches rather than timing out, and a tenant's first-ever autonomous mitigation requires Security Team sign-off and holds for validation rather than escalating on a clock.

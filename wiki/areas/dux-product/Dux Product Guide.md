...
| 4 | Config-change recommendations vs. patching | Control refinements live (per the capability spec) | Closed-loop validation → Gate 3 |
...
The 16-week Phase-1 calendar (starting Week 2 with the infrastructure skeleton) interlocks tightly with two external legal deadlines that don't move for anyone: an EU AI Act counsel opinion due around Week 6, ahead of the Article 50 transparency deadline, and a Langfuse DPA that has to land before production traces can flow, around Week 8. There's also a standing abort rule: if the golden set regresses by more than 2%, the inner reasoning framework gets switched, no negotiation.

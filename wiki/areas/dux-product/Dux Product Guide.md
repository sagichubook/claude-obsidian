...
Row 4 is worth a closer look, because two source documents disagree on its surface: the capability spec's own table calls control refinements "live" at Gate 1, but that reading doesn't survive a check against the decisions log. US-005 (Control Refinements) was explicitly deferred to the Gate-2 backlog by D-19 as a capacity fallback, with zero Gate-1 tasks scheduled against it, and D-23 later reconfirms the deferral still stands rather than reversing it. The capability spec's table cell was never updated to match, even though it was last reviewed after both decisions landed. The stepper panel and the read/query surface (`GET /controls/refinements`) do exist at Gate 1; the recommendation logic behind them is the part that's genuinely Gate 2, not Gate 1, whatever the capability table's own cell says.

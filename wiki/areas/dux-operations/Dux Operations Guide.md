...
That first staging game day is also where the DNS abort path gets its required live exercise: reverting a MinIO static-frontend deployment means restoring the prior build's object version straight from MinIO's own versioning history, and reverting a DNS record (when a DNS change, not just the frontend build, is what needs undoing) means calling the Cloudflare API to restore the prior `content`/`proxied` values captured under the standing pre-change-snapshot rule. Both paths run on credentials kept in Vault and rotated every 90 days (a scoped Cloudflare API token limited to DNS edits on the single `dux.io` zone, plus MinIO admin credentials), with platform on-call as the owner and the vendor consoles themselves as a break-glass fallback, not the primary path. The game day exists specifically to confirm those credentials actually authenticate, not merely that they're present in Vault.

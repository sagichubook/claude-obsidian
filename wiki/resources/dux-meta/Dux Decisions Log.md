...
### External review disposition (D-31, D-32)

**D-31 (pre-Agentic-RAG retrieval direction, later reversed by D-34):** stay on Postgres (pgvector + pgvectorscale) until ~100M vectors force a dedicated store; hybrid dense+BM25 search with a cross-encoder reranker, self-hosted `bge-reranker-v2-m3` as the residency-clean default. Phase-1 `rag_enabled=false` was correct at the time. Superseded once constrained decoding made Agentic RAG safe to re-enable (D-34).

**D-32 — canon holds against an outsider technical design document, on every conflicting axis.** Two external "TDD" documents proposed a different stack — AWS Bedrock AgentCore orchestration instead of direct Bedrock inference, Kafka/MSK instead of the Neon-era durable queue, Airbyte instead of the custom `vendor-contract.ts` ingestion layer, Pinecone/OpenSearch instead of pgvector, a Neo4j-first graph instead of Postgres-first, and a composite `DUX_SCORE` instead of the calibrated confidence ensemble. **None were adopted; no ADR or safety-spine change resulted.** Rationale: both TDDs were outsider reconstructions written without access to this corpus, and every divergence was either premature-scaling (Kafka, database-per-tenant, a dedicated vector vendor, before the traffic that would justify them) or positioning-breaking (a composite score is exactly the "yet another risk score" Dux's own public thesis attacks, and [[Dux Taxonomy and Controlled Vocabulary]] explicitly rejects it). The safety spine — governance-kernel gate chain, kill switch, HITL tiers, per-tenant child workflows — is built on the Temporal contract, so the AgentCore swap alone would have forced a rebuild of the entire AI-safety layer. Two narrow options were kept open for a real Gate-2 evaluation on their own merits, default posture unchanged: Airbyte as a long-tail ingestion accelerator ([[Open Items Register|OI-34]]) and per-tenant DB isolation as an enterprise tier ([[Open Items Register|OI-35]]).
